...
module.exports = function () {
  ...
  router.post('/classify', async (req, res) => {
    try {
      ...
      const text = await chat([
        { role: 'system', content: '거래 제목을 카테고리로 분류한다. 카테고리 이름만 출력한다. 카테고리: 식비, 교통, 쇼핑, 문화/여가, 의료, 통신, 주거, 교육, 기타' },
  ...
  router.post('/chat', async (req, res) => {
    try {
      ...
      const reply = await chat([
        { role: 'system', content: '당신은 Budget Buddy AI 재무 상담사입니다. 사용자의 소비 데이터를 분석하고 실용적인 조언을 제공합니다. 항상 한국어로 친근하고 간결하게 150자 이내로 답변하세요.' },
        { role: 'user', content: ctx ? `[재무 데이터]\n${ctx}\n\n[질문] ${message}` : message },
      ]);
      res.json({ reply });
    } catch (e) {
  ...
  router.post('/report', async (req, res) => {
    try {
      ...
      const report = await chat([
        { role: 'system', content: '당신은 개인 재무 분석 전문가입니다. 한국어로 친근하고 실용적인 월말 리포트를 작성합니다.' },
        { role: 'user', content:
  ...
  router.post('/suggest-budgets', async (req, res) => {
    try {
      ...
      const text = await chat([
        { role: 'system', content: '예산 추천 전문가. JSON만 출력. 다른 텍스트 없이.' },
        { role: 'user', content:
  ...
  router.post('/anomaly', async (req, res) => {
    try {
      ...
      const explanation = await chat([
        { role: 'system', content: '지출 이상 감지 분석가. 한국어로 60자 이내로 답변.' },
        { role: 'user', content: `거래: "${title}" ${amount}원 (${category}), 이 카테고리 평균: ${categoryAverage}원. 평균보다 많이 지출된 이유 분석 + 절약 팁 한 가지.` },
      ]);
